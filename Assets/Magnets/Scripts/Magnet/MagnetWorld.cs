    ...
    void OnDrawGizmos()
    {
        ...
        for (int i = 0; i < selectedMagnets.Length; i++)
        {
            ...
            for (int j = 0; j < magCount; j++)
            {
                ...
                //Only apply magnetic force if one of the objects is selected, negating two magnets from acting on each other without user input.
                if (!m1.Selected && !m2.Selected)
                    continue;
